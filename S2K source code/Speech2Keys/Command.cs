...
namespace Speech2Keys
{
	...
	public class Command
	{
		...
		public bool isValid(out string error)
		{
			...
			{
				error += "Invalid command name" + Environment.NewLine;
				isValid = false;
			}
				
			if (keyPhrases.Count == 0)
			{
				error+= "No phrases assigned" + Environment.NewLine;
				isValid = false;
			}
			...
			foreach (var k in keyPhrases)
				...
				{
					error+="Phrases "+ k + " is not unique" + Environment.NewLine;
					isValid = false;
				}	
			
			for (int i = 0; i<responses.Count; ++i)
				if (responses.LastIndexOf(responses[i]) != i)
					error+= "Info: Response '" + responses[i] + "' found more than once. (This may be intentional to have some responses occur more frequently)" + Environment.NewLine;
